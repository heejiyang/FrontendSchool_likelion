...
console.log(p); // Promise {<pending>} (마무리가 안되어있기 때문에) Promise { <fulfilled>: 'h' }
...
let p = new Promise(function(resolve, reject){
    ...
    reject('hello world');
        ...
        alert('catch 실행!!'); // catch 실행!! 실패할 경우 출력 값
    })
...
let p = new Promise(function(resolve, reject){
        ...
        alert(메시지); //hello
        throw Error('Error 발생! 경고경고!'); //error 던지기
        return 메시지[0];
    }).then(메시지 => {
        alert(메시지);
        return 메시지;
        ...
        console.log(메시지);
        alert('catch 실행!!'); //error만나면 catch 실행
    })
console.log(p);
...
let p = new Promise(function(resolve, reject){
        ...
        throw Error('Error 발생! 경고경고!');
            ...
            console.log(메시지);
            alert('catch 이후에 then이후에 catch실행!!');
    })
console.log(p);

// 일부러 시간이 좀 걸리게 작업을 해보도록 하자
let p = new Promise(function(resolve, reject){
...
let p = new Promise(function(resolve, reject){
    ...
    alert('catch 이후에 finally'); // but 관습적으로 마지막에 쓰는것이지 뒤에 이어붙이는 것이 안되는 것은 아니다.
    throw Error('Error 발생! 경고경고!');
}).catch(메시지 => {
    console.log(메시지);
...
fetch('http://test.api.weniv.co.kr/mall')
    ...
    .catch(메시지 => {
        // alert(메시지)
        alert('홈페이지가 정상적으로 작동하지 않고 있습니다. 조치중이니 잠시만 기다려주시길 바랍니다.');
        console.log(메시지);
    })
...
// 모던 자바스크립트 예제
let p = new Promise(function(resolve, reject) {
    setTimeout(() => resolve(1), 10000); // (*) resolve(1)의 인자가 다음 then의 result로 10초 뒤에 전달된다.
});
console.log('hello world');
// code 1000줄
