...
* nullish 병합 연산자(??)
    ```js
    let result1;
    let result2 = result1 ?? 100; //result2 //100
...
- String(문자열)
    ...
    * 메서드 :
        ...
        * str.includes()
        * str.slice()
        * str.splice() //배열에 있는 매서드이다. str에는 없다.
        * str.split()
        * str.substring()
        * str.substr()
        * str.toLowerCase()
        ...
        * str.repeat()
        ...
        'hello'.repeat(100)
        '0'.repeat(100)
        '5'.repeat(100).split('').map(Number) // 콜백함수가 Number로 들어갔다.
        ```
...
- Array(배열) : object
    ...
    * 메서드 : 
        ...
        * filter
        * push / pop - mutable
        * slice - immutable(원본 수정되지 않는 것)
        * splice - mutable(원본 수정)
        * reduce - immutable
        * join
        * indexOf
        * includes
    ...
    Array(100).fill(0).map((value, index)=> value + index) //0~100까지 원하는 숫자 한번에 채우기 위해ㄴ
    ```
...
- Object(객체)
    * 형태 : 
        ...
        let x=1, y=2, z=3
        let object = {x, y, z} // {x: 1, y: 2, z: 3} 객체도 구조분해할당이 가능하다.
        ```
